---
nanoid: "5hE8Ey"
doi: "10.17613/ndebt-83j29"
atproto: 'at://did:plc:e24okfpxr7ctcbmruijop5gp/site.standard.document/3mv2oaagg4p2w'
layout: archives.njk
article_number: '13'
volume: '24'
issue: '2'
pages: 317-322
title: 'Other Forms of Dwelling: A Dalit – Feminist Perspective'
author: 'Shrutika Lakshmi'
affiliation: ''
description: "Lakshmi frames Dalit feminist values alongside Indigenous frameworks to show alternative forms of dwelling, relation, and resistance beyond colonial modernity."
keywords:
  - dalit-feminism
  - indigenous-values
  - decoloniality
  - indian-subcontinent
  - embodied-knowledge
  - alternative-dwelling
pdf: 'lakshmi.pdf'
date: 2026-03-03
subjects:
  - label: "South Asia"
    scheme: "FAST"
    identifier: "fst01244520"
    uri: "https://id.worldcat.org/fast/1244520"
    category: "geographic"
  - label: "Decolonial theory"
    scheme: "Homosaurus"
    identifier: "homoit0003616"
    uri: "https://homosaurus.org/v5/homoit0003616"
    category: "topical"
bibliographic_abstract: >-
  This paper is an attempt to bring together different values from different socio-cultural contexts together
  with an intention to weave a meaningful narrative. For this reason, this paper will focus on the ‘Indigenous
  Values’ as explained by Philip Arnold in his book The Urgency of Indigenous Values as a basic structural
  framework to think about the Dalit women of the Indian subcontinent in consonance with these values and
  urgency. Similar to the indigenous values are the values lived and experienced by these Dalit women who help
  us to explore the different frameworks of existing in this shared world. To be precise, this paper is an
  earnest attempt to talk about the other ways of being in the world through the urgency of Dalit values. It
  is in this sense that this paper attempts to bring forth a shared framework to work with. This is a
  secondary scholarship based on the ethnography of other renowned scholars like Philip P Arnold and Lucinda
  Ramberg.
---
This paper is an attempt to bring together different values from different socio-cultural contexts together with an intention to weave a meaningful narrative. For this reason, this paper will focus on the ‘Indigenous Values’ as explained by Philip Arnold in his book *The Urgency of Indigenous Values* as a basic structural framework to think about the Dalit women of the Indian subcontinent in consonance with these values and urgency. Similar to the indigenous values are the values lived and experienced by these Dalit women who help us to explore the different frameworks of existing in this shared world. To be precise, this paper is an earnest attempt to talk about the other ways of being in the world through the urgency of Dalit values. It is in this sense that this paper attempts to bring forth a shared framework to work with. This is a secondary scholarship based on the ethnography of other renowned scholars like Philip P. Arnold and Lucinda Ramberg.
